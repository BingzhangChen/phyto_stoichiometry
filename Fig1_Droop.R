...
lm_Nlim  <- lm(Growth ~ C2N * Temp, Nlim)
...
lm_Plim <- lm(Growth ~ C2P * Temp, Plim)
...
result[1, 'muN']   <- N_coef[1,1]
result[1, 'muP']   <- P_coef[1,1]
result[1, 'SDmuN'] <- N_coef[1,2]
result[1, 'SDmuP'] <- P_coef[1,2]
result[1, 'QN0']   <- -N_coef[2,1]/N_coef[1,1]
result[1, 'QP0']   <- -P_coef[2,1]/P_coef[1,1]
...
z <- mvrnorm(N, mu=MEAN, Sigma = VCOV) %>% 
  ...
  filter(C2P < 0)
QP0 <- -z[,2]/z[,1]
result[1, 'SDQP0'] <- sd(QP0) 

for (i in 2:nrow(result)){
  result[i, 'muN'] <- N_coef[i+1,1] + N_coef[1,1]
  result[i, 'muP'] <- P_coef[i+1,1] + P_coef[1,1]
  
  #Estimate SE of mumax
  dSE <- N_coef[i+1, 2]
  result[i, 'SDmuN'] <- sd2(dSE, N_coef[1,2]) 
  
  dSE <- P_coef[i+1, 2]
  result[i, 'SDmuP'] <- sd2(dSE, P_coef[1,2]) 
  
  result[i, 'QN0'] <- -(N_coef[i+5,1] + N_coef[2,1])/result[i, 'muN'] 
  result[i, 'QP0'] <- -(P_coef[i+5,1] + P_coef[2,1])/result[i, 'muP']  
