##########################Fig. 4############
CVol_lm <- step(lm(log(POC) ~ log(CellVol) + Temp + Lim +
                         log(CellVol):Temp +
                         log(CellVol):Lim  +
                         Temp:Lim + log(CellVol):Temp:Lim, data=Semi))
summary(CVol_lm)
anova(CVol_lm)
...
pdf('Fig4_Vol_PC.pdf', height = 5, width = 6)
op   <- par(font.lab=1,
            family  ="serif",
            mar     = c(3.5,3.5,.1,.1),
            mgp     = c(2.2,1.2,0),
...
for (i in 1:length(Temps)){
  z <- newx %>% 
    filter(Temp == Temps[i])
  points(z$CellVol, z$PC, type='l', col=i, lty=i)
}
par(op)
dev.off()
